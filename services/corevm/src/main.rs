...
extern "C" fn refine(start_address: u64, length: u64) -> (u64, u64) {
    ...
    for i in 0..segment_index+1 {
        ...
        let peek_result = unsafe {
            peek(new_machine_idx, segment_buf_page_address, child_page_address, PAGE_SIZE);
        };
        ...
        if i == segment_index {
            ...
            call_log(2, None, &format!(" segment_buf[0..8]={:?}", &segment_buf[0..8]));
    ...
    let expunge_result = unsafe {
        expunge(new_machine_idx as u64);
    };
    gas_result = unsafe { gas() };
    call_log(2, None, &format!("expunge {:?}: expect OK {:?}, got {:?} gas={:?}", new_machine_idx, OK, expunge_result, gas_result));

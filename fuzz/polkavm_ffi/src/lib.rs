//! FFI library for generating ProgramBlob from arbitrary data
//! Used by Go fuzz tests to generate valid PolkaVM program blobs

use arbitrary::{Arbitrary, Unstructured};
use polkavm_common::program::{asm, Instruction, InstructionSetKind, Reg};
use std::slice;

// Re-use the same operation types from fuzz_polkavm.rs
...
/// Encode a variable-length integer using JAM's E() encoding
fn encode_e(mut value: u64) -> Vec<u8> {
    if value < 128 {
    ...
    let first_byte = match l {
        1 => 128 + (value & 0x3F) as u8,
        2 => 192 + (value & 0x1F) as u8,
        3 => 224 + (value & 0x0F) as u8,
        4 => 240 + (value & 0x07) as u8,
        5 => 248 + (value & 0x03) as u8,
        6 => 252 + (value & 0x01) as u8,
        _ => 254,
    };
...
/// Build a JAM-format program blob from instructions
/// Format: E(j_size) + E(z) + E(c_size) + j_bytes + code + compressed_bitmask
fn build_jam_blob_internal(data: Vec<OperationKind>) -> Result<Vec<u8>, ()> {
    let instructions = transform_code(data);
    let isa = InstructionSetKind::Latest64;

    // Encode instructions to raw bytes
    let mut code = Vec::new();
    let mut bitmask_bits = Vec::new();
    let mut position: u32 = 0;

    for inst in &instructions {
        let mut buffer = [0u8; 16]; // Max instruction size
        let len = inst.serialize_into(isa, position, &mut buffer);

        code.extend_from_slice(&buffer[..len]);

        // Mark all bytes of this instruction in bitmask
        // First byte = 1 (instruction start), rest = 0
        for i in 0..len {
            bitmask_bits.push(if i == 0 { 1u8 } else { 0u8 });
        }
    ...
    // Compress bitmask: pack bits into bytes
    let bitmask_len = (bitmask_bits.len() + 7) / 8;
    let mut compressed_bitmask = vec![0u8; bitmask_len];
    for (i, &bit) in bitmask_bits.iter().enumerate() {
        if bit != 0 {
            compressed_bitmask[i / 8] |= 1 << (i % 8);
        }
    }

    // Build JAM blob: E(j_size) + E(z) + E(c_size) + code + compressed_bitmask
    // j_size = 0, z = 0 (no jump table for now)
    let mut blob = Vec::new();
    blob.extend(encode_e(0)); // j_size = 0
    blob.extend(encode_e(0)); // z = 0
    blob.extend(encode_e(code.len() as u64)); // c_size
    blob.extend(code);
    blob.extend(compressed_bitmask);

    Ok(blob)
}

fn build_program_blob_internal(data: Vec<OperationKind>) -> Result<Vec<u8>, ()> {
    // Use JAM format instead of PolkaVM native format
    build_jam_blob_internal(data)
}

/// Result structure for FFI
#[repr(C)]
pub struct BlobResult {
    /// Pointer to the blob data (caller must free with free_blob)
    pub data: *mut u8,
...
/// Build a ProgramBlob from arbitrary fuzzing data
///
/// # Safety
/// - `data` must be a valid pointer to `len` bytes
/// - The returned BlobResult.data must be freed with `free_blob`
#[no_mangle]
pub unsafe extern "C" fn build_program_blob(data: *const u8, len: usize) -> BlobResult {
    ...
    // Try to parse operations from the input
    let ops: Vec<OperationKind> = match Vec::arbitrary(&mut unstructured) {
        Ok(ops) => ops,
        Err(_) => {
            return BlobResult {
                data: std::ptr::null_mut(),
    ...
    // Limit the number of operations to avoid extremely large programs
    let ops: Vec<OperationKind> = ops.into_iter().take(1000).collect();

    match build_program_blob_internal(ops) {
        Ok(blob) => {
            let len = blob.len();
...
/// Free a blob allocated by build_program_blob
///
/// # Safety
/// - `data` must be a pointer returned by build_program_blob
/// - `len` must be the length returned by build_program_blob
#[no_mangle]
pub unsafe extern "C" fn free_blob(data: *mut u8, len: usize) {
    if !data.is_null() && len > 0 {
...
mod tests {
    ...
    fn test_build_blob() {
        let input = [0u8; 100];
        unsafe {
            let result = build_program_blob(input.as_ptr(), input.len());
            if result.error == 0 {
                assert!(!result.data.is_null());
                assert!(result.len > 0);
                free_blob(result.data, result.len);
            }
        }
    }
}
